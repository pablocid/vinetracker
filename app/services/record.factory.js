...
exports.RecordFactory = function (schema, newschema) {
    ...
    RecordConstructor.prototype.getIdsForEdit = function(){
        var self = this;
        return self.newschema.filter(x=>x.type==="schmAttrInputConf")
                .filter(x=>self.findValueByVarHelper(x.attributes, "id", "createble","boolean")? true || undefined || null === true : false)
                .map(x=>self.findValueByVarHelper(x.attributes,'id','attribute','reference'));
    }
    ...
    RecordConstructor.prototype.getInputAttr = function (attrId, key){
        var self = this;
        // el input tiene 4 keys fijos porque es el que se implementa: 2 dt string y 2 dt listofObj
